...
def runfof(id_file,b_link):
    ...
    print(np.mean(g.b))
    ginfos, l_keys  =   g.fof()
    np.save('ginfos_p%04d_%s.npy' % (int(id_file),b_link), ginfos)
    np.save('keys_sel_p%04d_%s.npy' % (int(id_file),b_link), l_keys)
     

def process_data(file_path):
    data=pd.read_csv(file_path,sep=',')
    # remove pmnan
    data=data.dropna(subset=['ra','dec','parallax','pmra','pmdec','phot_g_mean_flux','phot_bp_mean_flux','phot_br_mean_flux'])
    # select by mag_g   parallax (0.2 mas - 7 mas)
    data=data[data['phot_g_mean_flux']<=18 & data['parallax']<=7 & data['parallax']>=0.2]
    # select by pm < 30 mas/yr
    data=data[data['pmra']<30 & data['pmdec']<30]
    return data


if __name__ == '__main__':
    id_file=18 # 第xx个分区
    # ID 1325 106905 stars 0.1:0 cluster; 0.15:0 cluster;  0.2:12clusters;  0.25:58 clusters;  0.3:469 clusters;
